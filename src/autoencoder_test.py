...
all_test_sequences, test_labels = create_sequences(test_df, window_size, useful_sensors)
all_test_tensor = torch.FloatTensor(all_test_sequences)
...
with torch.no_grad():
    test_reconstructed = model(all_test_tensor)
    test_reconstruction_error = torch.mean((all_test_tensor - test_reconstructed) ** 2, dim=(1, 2))

test_reconstruction_error = test_reconstruction_error.numpy()

test_results_df = pd.DataFrame(test_labels, columns=["unit_id", "cycle"])
test_results_df["reconstruction_error"] = test_reconstruction_error

test_results_df = test_results_df.merge(rul_df, on="unit_id")

test_anomaly_flag = test_results_df["reconstruction_error"] > 0.3
test_results_df["anomaly_flag"] = test_anomaly_flag
